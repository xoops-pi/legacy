...
class Legacy_Installer extends Xoops_Installer_App
{
    ...
    protected function compat(& $message)
    {
        ...
        */
        ...
        foreach ($smileyMap as $code => $image) {
            $status = $status * (int) $xoopsDB->queryF("INSERT INTO " . $xoopsDB->prefix("smiles") . " (`code`, `smile_url`, `emotion`) VALUES (" . $xoopsDB->quote($code) . ", " . $xoopsDB->quote($image) . ", " . $xoopsDB->quote(constant("_LEGACY_SYSTEM_SMILEY_" . (++$i))) . ")");
        }
        if (!$status) {
        ...
        foreach ($rankList as $rank) {
            $status = $status * (int) $xoopsDB->queryF("INSERT INTO " . $xoopsDB->prefix("ranks") . " (`rank_title`, `rank_min`, `rank_max`, `rank_special`, `rank_image`) VALUES (" . $xoopsDB->quote(constant("_LEGACY_SYSTEM_RANK_" . (++$i))) . ", " . $rank['param'][0] . ", " . $rank['param'][1] . ", " . $rank['param'][2] . ", " . $xoopsDB->quote($rank['image']) . ")");
        }
        if (!$status) {
