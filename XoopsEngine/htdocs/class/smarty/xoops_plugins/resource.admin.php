...
/**
 * Smarty admin resource handler
 * -------------------------------------------------------------
 */
function smarty_resource_admin_source($tpl_name, &$tpl_source, &$smarty)
{
    if ($tplPath = smarty_resource_admin_getpath($tpl_name, $smarty)) {
...
function smarty_resource_admin_getpath($tpl_name, &$smarty)
{
    global $xoops;

    list($module, $template) = explode("/", $tpl_name, 2);
    $info = XOOPS::service('registry')->module->read($module);
    $path = XOOPS::registry("view")->resourcePath($info['path'] . "/{$module}/templates/admin/{$template}");
