 ...
 *
 * <code>
 * <{block blockName var1=val1 var2=val2}>
 * </code>
 */
function smarty_compiler_block($argStr, $compiler)
{
    global $xoops;
    ...
    if (strpos($argStr, "$") === false) {
        ...
        $block = XOOPS::registry('view')->Block($blockName, $params);
        //return empty($block['content']) ? '' : 'echo "' . str_replace('"', '\"', $block['content']) . '";';
        return empty($block['content']) ? '' : '?>' . $block['content'] . '<?php';
    } else {
        ...
        $str = "\$block = XOOPS::registry(\"view\")->Block({$blockName}, ";
        $str .= "array(" . implode(", ", $pars) . "));\n";
        $str .= "echo empty(\$block['content']) ? '' : \$block['content'];";
        return $str;
    }
}